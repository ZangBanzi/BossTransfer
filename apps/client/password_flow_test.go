...
func TestClientFirstLoginPasswordChangeFlow(t *testing.T) {
	...
	auth := websession.New(store, "client_session", "/login")
	...
	loginPageHandler(auth).ServeHTTP(loginPageResponse, loginPage)
	...
	auth.Require(http.HandlerFunc(clientPasswordPageHandler(auth))).ServeHTTP(passwordPageResponse, passwordPage)
	...
	if passwordPageResponse.Code != http.StatusOK ||
		...
		!strings.Contains(pageHTML, "/api/v1/client/auth/credentials") ||
		!strings.Contains(pageHTML, "location.href='/login'") {
		t.Fatalf("password page = %d body=%s", passwordPageResponse.Code, pageHTML)
	}

	assertClientPasswordGate(t, auth, cookie, http.MethodGet, "http://nas/", http.StatusSeeOther)
	assertClientPasswordGate(t, auth, cookie, http.MethodGet, "http://nas/api/v1/client/search?q=movie", http.StatusPreconditionRequired)
	...
	assertClientPasswordGate(t, auth, changedCookie, http.MethodGet, "http://nas/", http.StatusNoContent)
	assertClientPasswordGate(t, auth, changedCookie, http.MethodGet, "http://nas/api/v1/client/search?q=movie", http.StatusNoContent)
}

func assertClientPasswordGate(t *testing.T, auth *websession.Auth, cookie *http.Cookie, method, target string, want int) {
	t.Helper()
